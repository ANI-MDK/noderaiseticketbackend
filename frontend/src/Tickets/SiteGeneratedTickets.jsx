...
const SiteGeneratedTickets = () => {
  ...
  const breadItems = [
    { label: "Home", href: "#" },
    { label: "Site Generated Tickets", active: true },
  ];
  ...
  const columns = [
    // { name: "# Sl_No.", selector: (rows, index) => index + 1, sortable: true },
    {
      name: "Ticket No",
      selector: (row) => (
        <Link to="/sitegeneratedtickets/sitegeneratedticketsdetail" state={row}>
          {row.ticket_number}
        </Link>
      ),
      sortable: true,
    ...
    },
    {
      ...
      sortFunction: (rowA, rowB) => {
        return (
          statusOrder[rowA.ticket_status] - statusOrder[rowB.ticket_status]
        );
      },
    },
  ];

  const [sitegeneratedticketsList, setSitegeneratedticketsList] = useState([]);
  const [filteredTickets, setFilteredTickets] = useState(
    sitegeneratedticketsList
  );

  const url = `${apiUrl}/ticket/getsiteotherticketlist`;
  const token = localStorage.getItem("token");

  const fetchSiteGeneratedTicketList = async () => {
    try {
      ...
      if (allTicketRes.data.status === "success") {
        // console.log(allTicketRes.data.data);
        setSitegeneratedticketsList(allTicketRes.data.data);
    ...
    <div className="absolute w-full h-full flex flex-col items-start">
      ...
      <div className="flex-1 relative w-[98.45%]">
        ...
        <div className="h-auto bg-white border-[1px] border-black">
          {/* <input type="search" placeholder="Search here..." className="absolute lg:-mt-[13px] lg:-right-2 z-10 border-[1px] border-black bg-gray-200 rounded-md text-lg px-10 lg:py-1 xl:py-1 2xl:w-[20%] w-[250px]"/>
          <CiSearch className="search-icon absolute z-10 lg:-top-2 cursor-pointer -mx-1 pt-1" size={24}/> */}
        </div>
        <div className="absolute top-0 left-0 h-auto w-full rounded-sm mb-3 bg-white border-[1px] border-black">
          <DataTable
            title="Site Generated Tickets"
...
export default SiteGeneratedTickets;
