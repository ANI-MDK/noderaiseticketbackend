...
const AllDepartments = () => {
  ...
  const breadItems = [
    ...
    { label: "Departments", href: "/alldepartments", active: true },
  ];
  ...
  const columns = [
    {name: "Sl_No.", 
    selector: (rows, index) => index + 1,
    },
    {
      name: `Department`,
      selector: (rows) => rows.department_name,
      sortable: true,
    },
    {
      name: "Assignee Name",
      selector: (rows) =>
        rows.current_assigner_name === null
          ? "--N/A--"
          : rows.current_assigner_name,
      ...
      cell: (row) => (
        ...
        >
          ...
          <svg
            ...
            viewBox="0 0 512 512"
            onClick={() =>
              navigate("/alldepartments/update", { state: { row } })
            } // Edit action handler
          >
            <title>Edit</title>
          ...
          <svg
            ...
            viewBox="0 0 448 512"
            onClick={() => handleDelete(row.department_id)} // Delete action handler
          >
            <title>Delete</title>
          ...
          <svg
            ...
            viewBox="0 0 448 512"
            onClick={() =>
              navigate("/alldepartments/assign", { state: { row } })
            }
          >
            <title>Assign</title>
  ...
  const searchQuery = (event) => {
    ...
    const val = departmentList.filter((item) => {
      return item.department_name.toUpperCase().includes(query);
    });
  ...
  const handleDelete = async (department_id) => {
    ...
    if (isConfirmed) {
      const url = `${apiUrl}/department/deletedepartment/${department_id}`;
      const token = localStorage.getItem("token");
            ...
            actions={
              <>
                {localStorage.getItem("ticketPermission") == "0" && (
                  <button
                    type="button"
                    onClick={() => navigate("/alldepartments/add")}
