...
const AllRoles = () => {
  ...
  const breadItems = [
    ...
    { label: "All Roles", href: "/allroles", active: true },
  ];
      ...
      cell: (row) => (
        ...
        >
          ...
          <svg
            ...
            viewBox="0 0 512 512"
            onClick={() => navigate("/allroles/update", { state: { row } })} // Edit action handler
          >
            <title>Edit</title>
          ...
          <svg
            ...
            viewBox="0 0 448 512"
            onClick={() => handleDelete(row.role_id)} // Delete action handler
          >
            <title>Delete</title>
  ...
  const searchQuery = (event) => {
    ...
    const val = roleList.filter((item) => {
      return item.role_name.toUpperCase().includes(query);
    });
  ...
  const handleDelete = async (role_id) => {
    ...
    if (isConfirmed) {
      const url = `${apiUrl}/role/deleterole/${role_id}`;
      const token = localStorage.getItem("token");
